...
public class HandlerUtil {
  ...
  public static final String PROP_RESULT_LIMIT = "enterprises.orbital.evekit.ref_model_ws.max_results";
  public static final int DEF_RESULT_LIMIT = 1000;
  ...
  public interface QueryCaller<A extends RefCachedData> {
    List<A> getList(long contid, int maxresults, boolean reverse, AttributeSelector at, AttributeSelector... others) throws IOException;
    long getExpiry();
  }
  ...
  public static <A extends RefCachedData> long handleStandardExpiry(Class<A> clz, ESIRefSyncEndpoint ep) {
    ...
    try {
      return Math.max(def, ESIRefEndpointSyncTracker.getLatestFinishedTracker(ep).getSyncEnd() + modelExpiry.get(clz) + shift);
  ...
  public static <A extends RefCachedData> Response handleStandardListRequest(AttributeSelector at, long contid, int maxresults, boolean reverse, QueryCaller<A> query, HttpServletRequest request, AttributeSelector... sels) {
